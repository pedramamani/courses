from toolbox import plot, process
...
def process_linear(number, show=False):
    ...
    current_func = process.fit_func(process.Function.line, time, ch1, ch1_errors)
...
def process_quadratic(number, show=False):
    ...
    peak_indices = process.index_peaks(-ch2, distance=10, prominence=0.05)
...
def process_trans_rf(number, show=False):
    ...
    if show:
        p.line(time / PRE.m, ch2, format_='.r', markersize=2)
        p.line(time / PRE.m, process.Function.rabi(time, *params), format_='k')
...
def process_trans_light(number, show=False):
    ...
    time, ch1, ch2 = set_.time(), set_.ch1(), set_.ch2()
    edges = process.index_edges(ch1, prominence=0.1)
    edge1, edge2 = edges[0], edges[-1]
    time, ch1, ch2 = time[edge1: edge2], ch1[edge1: edge2], ch2[edge1: edge2]

    # Decay fit to signal
    params, errors = process.fit(process.Function.decay, time, ch2, [Data.trans_rf.ch2_bin / 2] * len(time))
...
def fit_BvsI(show=False):
    ...
    Bfield = np.array([[0, rf_frequency_to_B(L.fRF[i], Rb87), rf_frequency_to_B(L.fRF[i], Rb85)]
                       for i, I in enumerate(L.Imain) if I == 0]).flatten()
    Bsweep = process.fit_func(process.Function.line, Isweep, Bfield, [1E-5] * len(Bfield), absolute_sigma=False)
    if show:
        p = plot.Plot()
    ...
    Bmain = process.fit_func(process.Function.line, Imain, Bfield, [1E-5] * len(Bfield), absolute_sigma=False)
    if show:
        p = plot.Plot()
...
def fit_fRFvsB(show=False):
    ...
    params87, errors87 = process.fit(process.Function.line, B87s, f87s, [1E-3] * len(f87s), absolute_sigma=False)
    params85, errors85 = process.fit(process.Function.line, B85s, f85s, [1E-3] * len(f85s), absolute_sigma=False)
    ...
    if show:
        p = plot.Plot()
        p.line(B87s / PRE.u, f87s / PRE.M, format_='ob', markersize=5)
        p.line(B87s / PRE.u, process.Function.line(B87s, *params87) / PRE.M, format_='--b')
        p.line(B85s / PRE.u, f85s / PRE.M, format_='*r', markersize=6)
        p.line(B85s / PRE.u, process.Function.line(B85s, *params85) / PRE.M, format_='r')
...
def fit_fRabivsVRF(show=False):
    ...
    params87, errors87 = process.fit(process.Function.line, T.VRF87, T.omega87, T.sigma_omega87)
    params85, errors85 = process.fit(process.Function.line, T.VRF85, T.omega85, T.sigma_omega85)
    print(f'g87 = {params87[0] * h / muB} +- {errors87[0] * h / muB}\n'
          f'g85 = {params85[0] * h / muB} +- {errors85[0] * h / muB}\n'
          f'offset87 (Hz) = {params87[1]} +- {errors87[1]}\n'
          f'offset85 (Hz) = {params85[1]} +- {errors85[1]}')
    ...
    if show:
        p = plot.Plot()
        p.line(T.VRF87, np.array(T.omega87) / PRE.k, format_='ob', markersize=5)
        p.line(T.VRF87, process.Function.line(T.VRF87, *params87) / PRE.k, format_='--b')
        p.line(T.VRF85, np.array(T.omega85) / PRE.k, format_='*r', markersize=6)
        p.line(T.VRF85, process.Function.line(T.VRF85, *params85) / PRE.k, format_='r')
